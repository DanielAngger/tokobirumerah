...
def create_pesanan_entry(request):
    ...
    if form.is_valid() and request.method == "POST":
        pesanan_entry = form.save(commit=False)
        pesanan_entry.user = request.user
        pesanan_entry.save()
...
def show_xml(request):
    data = PesananEntry.objects.filter(user=request.user)
...
@csrf_exempt
@require_POST
def add_pesanan_entry_ajax(request):
    pesanan = strip_tags(request.POST.get("pesanan"))
    keterangan = strip_tags(request.POST.get("keterangan"))
    quantitas = request.POST.get("quantitas")
    user = request.user

    new_pesanan = PesananEntry(
        pesanan=pesanan, keterangan=keterangan,
        quantitas=quantitas,
        user=user
    )
    new_pesanan.save()
